...
model_groups = {
    "Full 2-Gram Only": DELTA_FEATURES[:3],
    "Full 3-Gram Only": DELTA_FEATURES[3:6],
    "Full 4-Gram Only": DELTA_FEATURES[6:9],
    "Full 5-Gram Only": DELTA_FEATURES[9:],
    "Full 2-Gram + Full 3-Gram": DELTA_FEATURES[:6],
    "Full 2-Gram + Full 4-Gram": DELTA_FEATURES[:3] + DELTA_FEATURES[6:9],
    "Full 3-Gram + Full 4-Gram": DELTA_FEATURES[3:9],
    "Full 2-Gram + Full 5-Gram": DELTA_FEATURES[:3] + DELTA_FEATURES[9:],
...
for dataset, data_dir in DATASETS.items():
    ...
    for temp in TEMP_LIST:
        file_path = os.path.join(data_dir, f"kenlm_filtered_features_{dataset}_temp{temp}.csv") #nested loop for each temp
        ...
        df["label_num"] = df["label"].map({"human": 0, "ai": 1}) #label to binary
        ...
        y_pred = clf.predict(X_test) #predicted lables
        y_prob = clf.predict_proba(X_test)[:, 1] #Predicted Probabilities

        #Computes all main evaluation metrices
        ...
        #Generates and save confusion matrix visualization to PNG
    ...
    if results:
        ...
        #Model group evaulation
    if all_df:
        merged_df = pd.concat(all_df, ignore_index=True)
        model_results = [] #merges all temperature-based dataframes into single one for model group testing

        for name, features in model_groups.items():
            X = merged_df[features]
            y = merged_df["label_num"]

        #repeats model training + Prediction for this feature subset
        ...
        #Collects evaluation results and overfitting gap
        ...
        #Sort and saves the model group comparsion table to CSV.
